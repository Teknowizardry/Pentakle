...
// Copyright (c) 2013-2014 Pentakle Developers
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Pentakle: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Pentakle Signed Message:\n";
...
{
    // Pentakle: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 108 * 60; // Pentakle: every 108 minute/retarget
static const int64 nTargetSpacing = 9 * 60; // Pentakle: 9 minutes/block
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Pentakle: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // Pentakle: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x19b69bdb75325d34f3f0af38460c9329e15fcf01d84b1737cf4e5edb007a2b55");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "30.01.2014 Huffington Post: What Is Magick and Why On Earth Do I Want to Do It?";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PentakleMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PentakleMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PentakleMiner(CWallet *pwallet)
{
    printf("PentakleMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pentakle-miner");
    ...
    try { loop {
        ...
        printf("Running PentakleMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PentakleMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PentakleMiner, pwallet));
}
